...
public class BTree<K extends Comparable<K>, V>
  ...
  private int findInList (K key, List<Entry> list) {
    ...
    return list.size(); // Comment out this line.
  }
  ...
  private int findInListOfLists (K key, List<Entry> list) {
    ...
    return list.size()-1; // Comment out this line.
  }
	    ...
	    // If the list entries are leaves (contain data not lists):
	     if (list.get(0).bottom()) {
	      ...
	      // Use findInList to get the index of the key, if it is there.
	        ...
	        if(index < list.size() && list.get(index).key.compareTo(key) == 0  ){
  ...
  public V get (Object obj) {
    ...
    return (V)find(key,list).getValue(); // Comment out this line.
  }
  ...
  private void remove (K key, List<Entry> list) {
    ...
    // Else:
      ...
      // Get the entry at that index.
          Entry entry2 = list.get(sublistIndex);
      // Get its list, call it the sublist.
          List <Entry> sublist = entry2.getList();
      ...
      // sublist may have a new key at index 0 now.
		  ...
		  if(sublist.size() == 1){
			  ...
			  List<Entry> rightList = list.get(sublistIndex + 1).getList();
			  ...
			  if(sublist.size() < 4){
				  ...
				  List<Entry> right = split(sublist);
				  list.add(sublistIndex + 1, new Entry(right));
			  }
		  }
	  }
