  ...
  private static class Entry<K, V> implements Map.Entry<K, V> {
	...
	// element in that list.
    	if (key.equals(table[index].getKey())){
    		Entry<K, V> obj = table[index];
    		table[index] = obj.next;
    		size--;
    		return obj.getValue();
    	}
	...
	// element in that list.
    	...
    	for(Entry<K, V> obj = remove; !obj.next.getKey().equals(key); obj = obj.next)
    		remove = obj;

    	Entry<K, V> temp = remove.next;
    ...
    private void rehash (int newCapacity) {
      // IMPLEMENT
    	...
    	for (int i = 0; i < helper.length; i++){
    		for (Entry obj = helper[i]; obj != null; obj = obj.next)
    			put((K)obj.getKey(), (V)obj.getValue());
    	}
