...
public class OpenHashTable<K, V> extends AbstractMap<K, V> {
    ...
    private int find (Object key) {
      // IMPLEMENT
    	...
    	while (table[index] != null && (table[index] == DELETED || !key.equals(table[index].key))){
    		...
    		if (table[index] == DELETED && firstDele == -1)
    			firstDele = index;
    		index = (index + 1) % table.length;
    	}
    	if (table[index] != null || firstDele == -1 ){
    		return index;
    	}
      return firstDele;
    }
    ...
    private void rehash (int newCapacity) {
      // IMPLEMENT
    	Entry<K, V>[] helper = table;
    	table = new Node[newCapacity];
    	size = 0;
    	nonNulls = 0;
    	
    	for (int i = 0; i < helper.length; i++){
    		if (helper[i] != null && helper[i] != DELETED){
    			put(helper[i].getKey(), helper[i].getValue());
    		}
    	}
    }
    ...
    private class EntryIterator implements Iterator<Map.Entry<K, V>> {
      ...
      public Map.Entry<K, V> next () {
        // EXERCISE
    	  //if (counter == 0 && table[counter] != null)
    		//  return next;
    	  next = table[counter];
        return next;
      }
